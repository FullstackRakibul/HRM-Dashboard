...
const RaisedTicketListTable = () => {
  ...
  const fetchTicketData = async (Take = 5, Skip = 1) => {
    try {
      const responseRaisedTicketList = await AxiosInstance.get(
        `/api/Tickets/getTicketByCreator/088101`
      );

      const lists = configDataForTable(responseRaisedTicketList.data);
      if (lists.length) {
        setTickets(lists);
      }
...
export default RaisedTicketListTable;
